...
const Item = ({producto, detail, addCartNumber}) => {
    ...
    return (
        ...
        <div className= {detail ? "card text-white bg-black col-8 col-md-8 col-lg-6 mb-3 bg-black" : "card text-white bg-black col-8 col-md-4 col-lg-2 mb-3 bg-black"}>
          <img alt='Foto del item' style={detail ? {height: '500px'} : null} className="card-img-top" src={producto.img} />
          ...
          {detail ? <ItemCount stock={producto.stock} initial= {1} addCartNumber={addCartNumber}/> : <Link to={'/' + producto.categoria + '/' + producto.idFirebase} style={{ textDecoration: 'none' }} className="btn btn-danger mx-1">Detalle</Link>}
            </div>
          </div>
        </div>
